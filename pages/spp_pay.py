...
# File CSV khusus untuk pembayaran SPP
FILE_PATH = "spp_data.csv"
PAYMENT_TYPE = "SPP" # Definisikan jenis pembayaran di sini
...
def send_whatsapp_message(no_hp, nama, bulan_atau_ujian_list, jumlah, tanggal):
    pesan = (
        ...
        f"Kami menginformasikan bahwa pada *{tanggal}* ananda *{nama}* telah melakukan pembayaran {PAYMENT_TYPE} untuk bulan *{', '.join(bulan_atau_ujian_list)}* dengan nominal sebesar *Rp{jumlah:,.0f}*.\n\n"
...
bulan = st.multiselect("📌 Pilih Bulan Pembayaran",
                         ...
                         key=f"{PAYMENT_TYPE.lower()}_bulan")
...
if st.button(f"💾 Simpan & Kirim Notifikasi {PAYMENT_TYPE}", key=f"{PAYMENT_TYPE.lower()}_submit"):
    if nis and nama and bulan and jumlah > 0 and no_hp:
        tanggal_pembayaran = save_data(nis, nama, bulan, jumlah, no_hp)
        send_whatsapp_message(no_hp, nama, bulan, jumlah, tanggal_pembayaran)
        st.success(f"✅ Pembayaran {PAYMENT_TYPE} {nama} untuk bulan {', '.join(bulan)} berhasil dicatat & notifikasi terkirim.")
        st.rerun()
    else:
...
if not df.empty:
    ...
    if 'selected_edit_row_data_spp' not in st.session_state:
        st.session_state.selected_edit_row_data_spp = None
    if 'selected_index_spp' not in st.session_state: # New: Store original index
        st.session_state.selected_index_spp = -1
    ...
    if st.button(f"📝 Edit Data {PAYMENT_TYPE}", key=f"{PAYMENT_TYPE.lower()}_edit_button"):
        if selected_label:
            ...
            st.session_state.selected_edit_row_data_spp = df.loc[selected_index_df_display].to_dict()
            st.session_state.selected_index_spp = selected_index_df_display # Store the actual index
    ...
    if st.session_state.selected_edit_row_data_spp:
        row_to_edit = st.session_state.selected_edit_row_data_spp
        original_index_for_edit = st.session_state.selected_index_spp
        ...
        with st.form(f"edit_form_{PAYMENT_TYPE.lower()}"): # Key for the form
            ...
            edit_bulan_ujian = st.text_input("📌 Edit Bulan/Ujian (pisahkan dengan koma jika lebih dari satu)", row_to_edit["Bulan/Ujian"], key=f"edit_bulan_ujian_{PAYMENT_TYPE.lower()}")
            ...
            if submitted:
                ...
                if original_index_for_edit in current_df_for_update.index:
                    ...
                    st.session_state.selected_edit_row_data_spp = None # Clear selected data after successful edit
                    st.session_state.selected_index_spp = -1 # Clear original index
                    st.rerun()
                else:
    ...
    if st.button(f"🗑️ Hapus Data {PAYMENT_TYPE}", key=f"{PAYMENT_TYPE.lower()}_delete_button"):
        if selected_label: # Ensure a row is selected
            ...
            if selected_index_current_df in current_df_for_delete.index:
                ...
                st.session_state.selected_edit_row_data_spp = None # Clear selected data after successful deletion
                st.session_state.selected_index_spp = -1
                st.rerun()
            else:
